...
class MockKRXClient:
    ...
    def get_universe(self, as_of_date: str, market: str) -> pd.DataFrame:
        # 고정된 KOSPI 종목 3개 모킹
        return pd.DataFrame([
...
def test_golden_pipeline_regression(tmp_path, mock_dag):
    ...
    try:
        ...
        golden_df = pd.DataFrame([
            {"code": "005930", "name": "삼성전자", "market": "KOSPI", "close": 80000, "volume": 1000000, "market_cap": 400000000000000, "total_score": 0, "tier": 2},
            {"code": "000660", "name": "SK하이닉스", "market": "KOSPI", "close": 150000, "volume": 500000, "market_cap": 100000000000000, "total_score": 0, "tier": 3},
            {"code": "005380", "name": "현대차", "market": "KOSPI", "close": 200000, "volume": 300000, "market_cap": 40000000000000, "total_score": 0, "tier": 3},
        ])
        save_golden_set(golden_df, test_name)
        
    # 인덱스 초기화 및 컬럼 정렬 후 비교
    pd.testing.assert_frame_equal(
        final_output.reset_index(drop=True),
